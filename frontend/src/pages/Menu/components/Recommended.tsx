        ...
        <>
          {listToShow.length === 0 ? (
            ...
            <div
              className={`
                grid gap-3
                ${itemsToShowCount === 2 ? "grid-cols-1 grid-rows-2" : ""}
                ${itemsToShowCount === 4 ? "grid-cols-2 grid-rows-2" : ""}
                ${itemsToShowCount === 8 ? "grid-cols-4 grid-rows-2" : ""}
                h-[33.5rem] overflow-y-auto
              `}
            >
              {listToShow.map((activity) => (
                <div
                  key={activity.id}
                  className="flex flex-col items-start cursor-pointer overflow-hidden text-ellipsis whitespace-nowrap p-2"
